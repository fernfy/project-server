<template>
    <div>
        ...
        <form v-on:submit.prevent="Edit School">
            <div style="background-color:#98FB98; border-radius:10px; border: 1px rgb(0, 0, 0) solid;">
            Edit School 
</div>
            ...
            <button v-on:click="navigateTo('/school')">กลับ</button>
          
      </form>
        <hr>
        <div>
            <p>Name: {{ School.SchoolName }}</p>
            <p>price: {{ School.SchoolPrice }}</p>
            <p>Course: {{ School.Schoolcourse }}</p>
            <p>Class: {{ School.Schoolclass }}</p>
            <p></p>
        </div>
    </div>
</template>
